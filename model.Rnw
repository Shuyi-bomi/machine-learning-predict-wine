...
#head(trainDf)
# whitewinedis = whitewine[1:3,1:6]
# whitewinedis1 = whitewine[1:3,7:12]
# whitewinedisfi  = rbind(whitewinedis,whitewinedis1)
#   kable(whitewine[1:3,]) %>%
#   kable_styling("striped",full_width = F, position = "center")%>%
#   column_spec(1:12, width = "3in")
@
...
#phatL$glmmin = preglmmin$s0 #for lambda.min
rownames(accglm)='accuracy'
colnames(accglm)=c('lambda.min','lambda.1se')

#knitr::kable(accglm,caption="Accuracy" )
##so, choose lambda.min as best lambda
...
<<echo=FALSE,message=FALSE>>=
#knitr::kable(hyper_gridst[which.min(hyper_gridst$OOB_RMSE2sp),])
#hyper_gridst[which.min(hyper_gridst$OOB_RMSE3sp),]
...
#yhatpl = cbind(yhatpl,array(sapply(1:nrow(testDf),function(i) yhat[i,which.max(yhat[i,])] ))) #prob of choosing that class
#yhatpl = cbind(yhatpl,array(sapply(1:nrow(testDf),function(i) yhat[i,trainDf$quality[i]-2] )))
#colnames(yhatpl)[8:10]<- c("class","prob","prob(tclass)")
#plot(trainDf$quality,yhatpl[,10]) 
#abline(0,1/6,col="red")
@
...
#hyper_grid = read.table("rmserf.txt",header=TRUE,row.names=1) # says first column are rownames
...
#params <- list(booster = "gbtree", objective = "multi:softmax",num_class=7, eta=0.1, gamma=0.05, max_depth=3, min_child_weight=1, subsample=1, colsample_bytree=1)
...
#write.table(hyper_gridboost,"boostrmse.txt")
#hyper_gridboost = read.table("boostrmse.txt",header=TRUE,row.names=1) # says first column are rownames
...
#yhatpl = cbind(yhatst,array(sapply(1:nrow(testDf),function(i) which.max(yhatst[i,])))) #choose category
...
# rs <- roc.multist[['rocs']]
# plot.roc(rs[[1]],xlim=c(1,0),ylim=c(0,1), main="Single Tree",add=FALSE)
# sapply(2:length(rs),function(i) lines.roc(rs[[i]],col=i))
# legend(1, 1.8, c("class 1", "class 2", "class 3","class 4","class 5"), col = c(1, 2, 3,4,5), lwd = c(1,1,1,1,1),bg="transparent",cex=0.8)
...
# rs <- roc.multirf[['rocs']]
# plot.roc(rs[[1]],ylim=c(0.5,1),main="Random Forest")
# sapply(2:length(rs),function(i) lines.roc(rs[[i]],col=i))
# legend(1, 1.5, c("class 0", "class 1", "class 2"), col = c(1, 2, 3), lwd = c(1,1,1),bg="transparent",cex=0.9)
...
# rs <- roc.multiboost[['rocs']]
# plot.roc(rs[[1]],ylim=c(0.5,1), main="boosting")
# sapply(2:length(rs),function(i) lines.roc(rs[[i]],col=i))
# legend(1, 1.5, c("class 0", "class 1", "class 2"), col = c(1, 2, 3), lwd = c(1,1,1),bg="transparent",cex=0.9)
...
<<echo=FALSE,message=FALSE>>=
#single
#single_params = read.table("/Users/apple/Downloads/mratednn_single_1.txt",header=TRUE,row.names=1)
...
#lcnn = as.data.frame(liftcurve(probnn, qualityte))
...
# phatLLrf = vector("list",nfit)
# for(i in 1:nfit) {
# phatLLrf[[i]] = probrf[,i] }
# lift.many.plot(phatLLrf, qualityte)
